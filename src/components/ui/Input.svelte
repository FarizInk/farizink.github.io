<script lang="ts">
  ...
  const inputClasses = $derived([...baseInputClasses, sizeClasses, variantClasses, stateClasses, leftIconPadding, rightIconPadding, className].join(' '));
  ...
  <div class="relative">
    {#if leftIcon}
      <div class={leftIconClasses}>
        <svelte:component this={leftIcon} class="h-5 w-5 text-gray-400 dark:text-gray-500" />
      </div>
    {/if}
    ...
    {#if rightIcon}
      <div class={rightIconClasses}>
        <svelte:component this={rightIcon} class="h-5 w-5 text-gray-400 dark:text-gray-500" />
      </div>
    {/if}
  </div>
