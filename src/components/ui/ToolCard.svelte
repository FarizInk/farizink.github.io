...
>
  ...
  <div class="p-6">
    ...
    <div
      class="w-16 h-16 rounded-lg flex items-center justify-center mb-4 transition-all duration-300 {isHovered
        ? 'scale-110 shadow-lg'
        : ''}"
      class:gradient-red={tool.color === 'red' || tool.color === 'password-red'}
      class:gradient-blue={tool.color === 'blue' ||
        tool.color === 'hash-blue' ||
        tool.color === 'url-orange'}
    ...
    >
      {#if tool.icon}
        ...
        <Icon class="w-8 h-8 text-white drop-shadow-md transition-all duration-300" />
      {/if}
    </div>
    ...
    <div class="space-y-2 mb-4">
      {#each tool.features.slice(0, 3) as feature, index (index)}
    ...
    <div class="flex items-center justify-between">
      ...
      <div
        class="flex items-center text-blue-600 dark:text-blue-400 opacity-0 group-hover:opacity-100 transition-opacity duration-300"
      >
  ...
  {#if !tool.comingSoon && isHovered}
    <div
      class="absolute inset-0 bg-gradient-to-r from-blue-600/5 to-purple-600/5 pointer-events-none"
    ></div>
  {/if}
