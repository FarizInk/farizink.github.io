...
<main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 min-h-screen">
  ...
  {#if loading}
    <div
      class="flex flex-col items-center justify-center min-h-[300px] text-gray-600 dark:text-gray-400"
    >
      <div
        class="w-10 h-10 border-3 border-gray-300 dark:border-gray-600 border-t-blue-500 rounded-full animate-spin mb-4"
      ></div>
  ...
  {:else}
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
      {#each repositories as repo (repo.id)}
        <div
          class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-6 hover:border-gray-300 dark:hover:border-gray-600 hover:-translate-y-1 hover:shadow-lg transition-all duration-200 flex flex-col"
        >
          <div class="mb-4">
          ...
          <p class="text-gray-600 dark:text-gray-400 mb-4 leading-relaxed flex-grow">
            {repo.description}
          </p>
          ...
          {#if repo.topics && repo.topics.length > 0}
            <div class="flex flex-wrap gap-2 mb-4">
              {#each repo.topics.slice(0, 3) as topic (topic)}
                <span
                  class="bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 px-2 py-1 rounded-full text-xs font-medium"
                >
                  {topic}
                </span>
              {/each}
          ...
          <div
            class="flex justify-between items-center mt-auto pt-4 border-t border-gray-200 dark:border-gray-700 text-sm text-gray-600 dark:text-gray-400"
          >
