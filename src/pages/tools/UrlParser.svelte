<script lang="ts">
  ...
  function loadSampleUrl() {
    urlInput = 'https://example.com:8080/path/to/page?query=search&category=tools&sort=popular#section-1';
    parseUrl();
  }
...
<div class="max-w-6xl mx-auto p-6">
  ...
  <div class="mb-8">
    ...
    <div class="text-center mb-8">
      <div class="inline-flex items-center justify-center w-20 h-20 bg-gradient-to-br from-lime-500 to-green-600 rounded-2xl mb-4">
      ...
      <p class="text-xl text-gray-600 dark:text-gray-400 max-w-2xl mx-auto">
        Parse URLs and extract components including domain, path, query parameters, and hash fragments.
      </p>
    </div>
  </div>
  ...
  {#if parsedUrl}
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
      ...
      <!-- Complete Components -->
      <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
        <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Complete Components</h3>
    ...
    {#if parsedUrl.searchParams.length > 0}
      <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 mb-6">
        <div class="flex justify-between items-center mb-4">
          ...
          <span class="text-sm text-gray-500 dark:text-gray-400">{parsedUrl.searchParams.length} parameters</span>
        </div>
          ...
          <table class="w-full">
            <thead>
              <tr class="border-b border-gray-200 dark:border-gray-700">
                <th class="text-left py-2 px-3 text-sm font-medium text-gray-700 dark:text-gray-300">Parameter</th>
                <th class="text-left py-2 px-3 text-sm font-medium text-gray-700 dark:text-gray-300">Value</th>
                <th class="text-center py-2 px-3 text-sm font-medium text-gray-700 dark:text-gray-300">Actions</th>
              </tr>
            </thead>
            <tbody>
              {#each parsedUrl.searchParams as param, index}
                <tr class="border-b border-gray-100 dark:border-gray-800">
                  <td class="py-2 px-3">
                    <code class="text-sm text-gray-900 dark:text-white">{param.key}</code>
                  </td>
                  <td class="py-2 px-3">
                    <code class="text-sm text-gray-900 dark:text-white truncate max-w-xs">{param.value}</code>
                  </td>
                  <td class="py-2 px-3 text-center">
                    <div class="flex justify-center gap-2">
                      <button
                        onclick={() => copyToClipboard(param.key, `param-key-${index}`)}
                        class="p-1 text-gray-500 hover:text-gray-700 dark:hover:text-gray-300"
                      ...
                      <button
                        onclick={() => copyToClipboard(param.value, `param-value-${index}`)}
                        class="p-1 text-gray-500 hover:text-gray-700 dark:hover:text-gray-300"
  ...
  <div class="mt-12 grid grid-cols-1 md:grid-cols-3 gap-6">
    <div class="p-6 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
      <div class="w-12 h-12 bg-lime-100 dark:bg-lime-900/20 rounded-lg flex items-center justify-center mb-4">
