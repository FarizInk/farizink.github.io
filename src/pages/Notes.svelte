          ...
          <div class="flex flex-col gap-2">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
              ...
              <div class="flex items-center gap-3">
                <h1
                  class="text-lg sm:text-xl font-bold text-secondary-900 dark:text-white tracking-tight"
                >
                  My Notes
                </h1>
                {#if $totalCount > 0}
                  <div class="flex items-center gap-2">
                    ...
                    <span
                      class="text-xs text-secondary-500 dark:text-secondary-400 hidden sm:inline"
                    >
                      {$notes.length} shown
                    </span>
                  </div>
    ...
    {:else if viewMode === 'detail'}
      ...
      {:else if singleNote}
        ...
        <div class="max-w-4xl mx-auto">
          <!-- Header -->
          <div
            class="bg-white dark:bg-secondary-800 rounded-2xl shadow-lg border border-secondary-200 dark:border-secondary-700 px-6 py-4 mb-6"
          >
            <div class="flex items-start justify-between gap-4">
              <div class="flex-1 min-w-0">
                ...
                <div
                  class="flex items-center gap-4 text-sm text-secondary-500 dark:text-secondary-400"
                >
                  <div class="flex items-center gap-1">
                    <Calendar class="w-4 h-4" />
                    <span
                      >Created: {new Date(singleNote.created_at).toLocaleDateString('en-US', {
                        year: 'numeric',
                        month: 'short',
                        day: 'numeric'
                      })}</span
                  ...
                  <div class="flex items-center gap-1">
                    ...
                    <span
                      >Updated: {new Date(singleNote.updated_at).toLocaleDateString('en-US', {
                        year: 'numeric',
                        month: 'short',
                        day: 'numeric'
                      })}</span
          ...
          <!-- Content -->
          <div
            class="bg-white dark:bg-secondary-800 rounded-2xl shadow-lg border border-secondary-200 dark:border-secondary-700 p-6 space-y-6"
          >
            <!-- Link Section -->
            {#if singleNote.link}
              <div
            ...
            {#if singleNote.description}
              <div>
                ...
                <div
                  class="prose prose-sm max-w-none dark:prose-invert text-secondary-700 dark:text-secondary-300"
                >
            ...
            {#if singleNote.files && singleNote.files.filter( f => f.mime_type.startsWith('image/') ).length > 0}
              <div>
                ...
                >
                  <svg
                    class="w-5 h-5 text-yellow-600 dark:text-primary-400"
                    viewBox="0 0 24 24"
                    fill="none"
                    stroke="currentColor"
                    stroke-width="2"
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {#each singleNote.files.filter( f => f.mime_type.startsWith('image/') ) as file, index (file.id + '-' + index)}
                    <div
                      class="group relative bg-secondary-50 dark:bg-secondary-700 rounded-lg overflow-hidden hover:shadow-lg transition-all"
                    >
            ...
            {#if singleNote.files && singleNote.files.filter(f => !f.mime_type.startsWith('image/')).length > 0}
              <div>
                ...
                <div class="space-y-3">
                  {#each singleNote.files.filter(f => !f.mime_type.startsWith('image/')) as file, index (file.id + '-' + index)}
                    <div
                      class="flex items-center gap-4 p-4 bg-secondary-50 dark:bg-secondary-700 rounded-lg border border-secondary-200 dark:border-secondary-600"
                    >
                      <div
                        class="w-12 h-12 bg-secondary-200 dark:bg-secondary-600 rounded-lg flex items-center justify-center"
                      >
                        <svg
